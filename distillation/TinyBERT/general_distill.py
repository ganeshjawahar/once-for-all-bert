    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        with tqdm(total=len(train_dataloader), desc="Epoch {}".format(epoch)) as pbar:
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                print(len(teacher_reps), len(teacher_reps), len(teacher_atts), teacher_reps[0].size(), teacher_atts[0].size())
                print(len(student_reps), len(student_atts), student_reps[0].size(), student_atts[0].size())
